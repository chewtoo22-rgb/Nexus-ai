...
export interface Env {
  AI: Ai; BROWSER: Fetcher; VECTORIZE: VectorizeIndex; DB: D1Database; BUCKET: R2Bucket; CACHE: KVNamespace; SESSIONS: KVNamespace; AI_SEARCH: any;
  NEXUS_AGENT: DurableObjectNamespace; BUILDER_AGENT: DurableObjectNamespace; RESEARCHER_AGENT: DurableObjectNamespace; CREATIVE_AGENT: DurableObjectNamespace; ANALYST_AGENT: DurableObjectNamespace;
  NEXUS_MCP: DurableObjectNamespace; VOICE_AGENT: DurableObjectNamespace; SANDBOX: DurableObjectNamespace; RAG_WORKFLOW: Workflow; DOC_QUEUE: Queue<any>; ASSETS: Fetcher;
}

const AGENT_MAP = {
  nexus: "NEXUS_AGENT", sirius: "ANALYST_AGENT", ana: "BUILDER_AGENT", nova: "RESEARCHER_AGENT", creative: "CREATIVE_AGENT",
  builder: "BUILDER_AGENT", researcher: "RESEARCHER_AGENT", analyst: "ANALYST_AGENT",
} as const;

const AGENT_PROMPTS: Record<string, string> = {
  nexus: "You are Nexus, a frontier-grade AI.", sirius: "You are Sirius, Mission Control and the primary orchestrator.",
  ana: "You are Ana, the autonomous Builder Agent.", nova: "You are Nova, the autonomous Research Agent.", creative: "You are the Creative Agent.",
  builder: "You are Ana, the autonomous Builder Agent.", researcher: "You are Nova, the autonomous Research Agent.", analyst: "You are Sirius, Mission Control and the primary orchestrator.",
};
...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    if (!path.startsWith("/api/") && path !== "/mcp" && !path.startsWith("/voice")) { if (env.ASSETS) return env.ASSETS.fetch(request); return new Response("Static assets not configured. Add assets.directory in wrangler.jsonc.", { status: 503 }); }
    const clientIP = request.headers.get("CF-Connecting-IP") || "unknown"; const rl = await checkRateLimit(env, clientIP); const rlH = getRateLimitHeaders(rl);
    if (!rl.allowed) return json({ error: "Rate limit exceeded", remaining: 0 }, 429, { "Retry-After": String(Math.max(1, Math.ceil((rl.resetAt - Date.now()) / 1000))), ...rlH });
    ...
    if (path.startsWith("/api/agent/") && request.headers.get("Upgrade") === "websocket") {
      const agentType = path.split("/")[3]; const bindingName = AGENT_MAP[agentType as keyof typeof AGENT_MAP]; if (!bindingName) return new Response("Unknown agent", { status: 404 });
      const agentId = url.searchParams.get("id") || "default"; return (env as any)[bindingName].get((env as any)[bindingName].idFromName(agentId)).fetch(request);
    }
    if (needsAuth(path, request.method)) { const session = await authenticateRequest(request, env); if (!session) return json({ error: "Authentication required" }, 401, rlH); }
    const ok = (data: unknown, status = 200) => json(data, status, rlH);
    try { return await handleApi(request, env, path, url, ok, rlH); } catch (err) { const res = httpError(err); const headers = new Headers(res.headers); for (const [k, v] of Object.entries(rlH)) headers.set(k, v); return new Response(res.body, { status: res.status, headers }); }
  },
...
async function handleApi(request: Request, env: Env, path: string, url: URL, ok: (data: unknown, status?: number) => Response, rlH: Record<string, string>): Promise<Response> {
  ...
  if (path === "/api/chat" && request.method === "POST") {
    const body = await parseJson<{ message?: string; agent?: string; sessionId?: string; model?: string; images?: string[]; stream?: boolean }>(request); const message = String(body.message || "").trim(); if (!message) return ok({ error: "message required" }, 400);
    const agentType = body.agent || "nexus"; const bindingName = AGENT_MAP[agentType as keyof typeof AGENT_MAP]; if (!bindingName) return ok({ error: "Unknown agent" }, 400);
    const modelKey = agentType === "sirius" ? "analyst" : agentType === "ana" ? "builder" : agentType === "nova" ? "researcher" : agentType;
    const selectedModel = body.model || AGENT_MODELS[modelKey as keyof typeof AGENT_MODELS].primary; const sid = body.sessionId || crypto.randomUUID();
    if (body.stream) {
    ...
    return (env as any)[bindingName].get((env as any)[bindingName].idFromName(sid)).fetch("https://do/chat", { method: "POST", body: JSON.stringify({ content: message, conversationId: sid, images: body.images, model: body.model }), headers: { "Content-Type": "application/json" } });
  }

  if (path === "/api/missions" && request.method === "GET") return ok(await listMissions(env.DB));
  if (path === "/api/missions" && request.method === "POST") {
    const body = await parseJson<any>(request); const goal = String(body.goal || "").trim(); if (!goal) return ok({ error: "goal required" }, 400);
    const mission = await createMission(env.DB, goal, body.projectId || null); const rawSteps = Array.isArray(body.steps) ? body.steps : []; const steps = [];
    for (let i = 0; i < rawSteps.length; i++) { const step = rawSteps[i] || {}; if (!step.task) continue; steps.push(await addMissionStep(env.DB, mission.id, i, normalizeAgent(step.agent), String(step.title || `Step ${i + 1}`), String(step.task))); }
    if (steps.length) await setMissionStatus(env.DB, mission.id, "running"); return ok({ ...mission, status: steps.length ? "running" : mission.status, steps }, 201);
  }
  if (path.startsWith("/api/missions/") && path.endsWith("/status") && request.method === "PATCH") {
    const id = path.split("/")[3]; const body = await parseJson<any>(request); const status = String(body.status || "") as MissionStatus;
    if (!["queued", "planning", "running", "completed", "failed", "cancelled"].includes(status)) return ok({ error: "invalid status" }, 400);
    await setMissionStatus(env.DB, id, status, body.result ?? null, body.error ?? null); return ok(await getMission(env.DB, id));
  }
  if (path.startsWith("/api/missions/") && path.endsWith("/steps") && request.method === "POST") {
    const id = path.split("/")[3]; const body = await parseJson<any>(request); if (!body.task) return ok({ error: "task required" }, 400); const current = await getMission(env.DB);
    const index = Number.isInteger(body.index) ? body.index : current.steps.length; const step = await addMissionStep(env.DB, id, index, normalizeAgent(body.agent), String(body.title || `Step ${index + 1}`), String(body.task)); await setMissionStatus(env.DB, id, "running"); return ok(step, 201);
  }
  if (path.startsWith("/api/missions/") && path.includes("/steps/") && path.endsWith("/status") && request.method === "PATCH") {
    const parts = path.split("/"); const stepId = parts[5]; const body = await parseJson<any>(request); const status = String(body.status || "") as any;
    if (!["queued", "running", "completed", "failed"].includes(status)) return ok({ error: "invalid step status" }, 400); await setStepStatus(env.DB, stepId, status, body.result ?? null); return ok({ ok: true });
  }
