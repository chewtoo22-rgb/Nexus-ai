...
export class NexusMCP extends McpAgent {
  server = new McpServer({ name: "nexus-ai", version: "3.0.4" });
  async init() {
    ...
    this.server.tool("search_knowledge", "Search knowledge base (empty unless authenticated ingest populated it)", { query: z.string() }, async ({ query }) => {
