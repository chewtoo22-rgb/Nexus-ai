...
type AgentLike = Agent & { systemPrompt: string; agentType: string; models: any; state: any; env: any };

export class NexusAgent extends Agent {
  systemPrompt = NEXUS_PROMPT;
  agentType = "nexus";
  models = AGENT_MODELS.nexus;
  async onConnect(c: any, ctx: ConnectionContext) { connectAgent(this as any, c, ctx); }
...
function connectAgent(agent: AgentLike, conn: any, context: ConnectionContext): void {
  const userId = context.request.headers.get("X-Nexus-User-Id");
  if (!userId) {
    conn.close(1008, "Authentication required");
    return;
  }
  agent.state = { ...(agent.state || {}), userId, history: agent.state?.history || [] };
...
async function handleAgentMessage(agent: AgentLike, conn: any, message: WSMessage) {
  ...
  if (stream) {
    ...
    await streamChat({
      ...
      onToolResult: (tool, result) => conn.send(JSON.stringify({ type: "tool_result", tool, result: String(result).slice(0, 500) })),
  ...
  while (rounds-- > 0) {
    ...
    if (toolCalls?.length) {
      ...
      for (const tc of toolCalls) {
        ...
        const tr = await executeTool(fnName, fnArgs, agent.env, { storage: (agent as any).storage, userId: agent.state?.userId });
