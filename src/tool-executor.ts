...
const DELEGATE_PROMPTS: Record<string, string> = {
  builder: "You are Ana, a senior software architect. Produce a concrete implementation-oriented answer.",
  researcher: "You are Nova, a research agent. Distinguish facts from inference and cite uncertainty.",
  creative: "You are the Creative Agent. Produce a polished, constraint-faithful draft.",
...
export async function executeTool(toolName: string, args: any, env: any, ctx?: any): Promise<ToolCallResult> {
  switch (toolName) {
    case "web_search": {
      ...
      return await browserFetchMarkdown(env, `https://html.duckduckgo.com/html/?q=${q}`);
    }
    ...
    case "browser_action":
      return { result: "browser_action is not implemented. Use browser_navigate, browser_extract, or browser_screenshot." };
    case "analyze_image": {
      const r = await env.AI.run(MODELS.vision.fast, { prompt: args.question, image: args.image_url });
      return { result: (r as any).description || (r as any).response || JSON.stringify(r) };
    }
    case "generate_image": {
      const mm: Record<string, string> = {
        "flux-2-dev": MODELS.imageGen.flagship,
        "flux-2-klein-4b": MODELS.imageGen.fast,
        "flux-2-klein-9b": MODELS.imageGen.balanced,
        "flux-1-schnell": MODELS.imageGen.schnell,
        leonardo: MODELS.imageGen.leonardo,
        phoenix: MODELS.imageGen.phoenix,
      };
      ...
      if (img instanceof Uint8Array || img instanceof ArrayBuffer) {
        ...
        return { result: `Image saved: ${key}`, artifact: { type: "image", title: args.prompt, r2_key: key } };
      }
    ...
    case "ai_search": {
      try {
        return { result: JSON.stringify(await env.AI_SEARCH.search({ query: args.query })) };
      } catch {
        return { result: "AI Search not configured." };
      }
    }
    ...
    case "create_artifact":
      return { result: `Artifact "${args.title}" created.`, artifact: { type: args.type, title: args.title, content: args.content, language: args.language } };
    case "translate": {
      const r = await env.AI.run(MODELS.translation, { text: args.text, target_lang: args.target_lang, source_lang: "en" });
      return { result: (r as any).translated_text || JSON.stringify(r) };
    }
    case "text_to_speech": {
    ...
    case "save_memory": {
      if (ctx?.storage) await ctx.storage.put(`memory:${args.key}`, args.value);
      return { result: `Memory saved: ${args.key}` };
    }
    case "get_memory": {
      if (ctx?.storage) {
        const v = await ctx.storage.get(`memory:${args.key}`);
        return { result: v || `No memory for: ${args.key}` };
      }
      return { result: "Memory not available." };
    }
    case "delegate_to_agent": {
      const agent = String(args.agent || "");
      const task = String(args.task || "").trim();
      if (!DELEGATE_PROMPTS[agent] || !task) return { result: "delegate_to_agent requires agent (builder|researcher|creative|analyst) and task." };
      const model = AGENT_MODELS[agent as keyof typeof AGENT_MODELS]?.primary || MODELS.chat.fast;
      const r = await env.AI.run(model, {
        messages: [
          { role: "system", content: DELEGATE_PROMPTS[agent] },
          { role: "user", content: task },
        ],
        max_tokens: 2048,
        temperature: 0.4,
      });
      return { result: (r as any).response || JSON.stringify(r) };
    }
    case "run_code": {
...
async function browserFetchMarkdown(env: any, url: string): Promise<ToolCallResult> {
  ...
  try {
    const r = await env.BROWSER.quickAction("markdown", { url: safe.toString() });
    return { result: (await r.text()).slice(0, 8000) };
  } catch {
    const r = await fetch(safe.toString(), { headers: { "User-Agent": "NexusAI/3.0" }, redirect: "manual" });
    if (r.status >= 300 && r.status < 400) return { result: "Redirects are not followed." };
    const h = await r.text();
    const sanitized = stripDangerousBlocks(h);
    return { result: sanitized.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 8000) };
  }
}
...
async function browserExtract(env: any, url: string, selector: string): Promise<ToolCallResult> {
  const safe = assertPublicHttpUrl(url);
  try {
    const r = await env.BROWSER.quickAction("json", { url: safe.toString(), selector });
    return { result: (await r.text()).slice(0, 8000) };
  } catch {
    return { result: "Failed to extract." };
  }
}
